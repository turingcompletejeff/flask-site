...
class TestMinecraftCommand:
    """Test suite for MinecraftCommand model with cross-database StringArray type."""

    def test_minecraft_command_creation(self, db):
        """Test basic MinecraftCommand creation."""
        command = MinecraftCommand(
            command_name='give',
            options=['player1', 'diamond', '64']
        )
        ...
        assert command.options == ['player1', 'diamond', '64']

    def test_minecraft_command_options_array(self, db):
        """Test that options field properly stores array data."""
        command = MinecraftCommand(
            command_name='tp',
            options=['player1', '100', '64', '-200']
        )
        db.session.add(command)
        db.session.commit()
        ...
        assert retrieved.options == ['player1', '100', '64', '-200']
        assert len(retrieved.options) == 4

    def test_minecraft_command_empty_options(self, db):
        """Test MinecraftCommand with empty options array."""
        command = MinecraftCommand(
            command_name='list',
            options=[]
        )
    ...
    def test_minecraft_command_to_dict(self, db):
        ...
        assert result['options'] == ['player1', 'griefing']
